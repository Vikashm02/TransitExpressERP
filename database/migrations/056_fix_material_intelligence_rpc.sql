-- ==========================================================
-- Migration: 056_fix_material_intelligence_rpc
-- Module:    Material Intelligence V1 — SQL runtime fix
--
-- Replaces public.get_material_intelligence (same signature) after
-- migration 055. Does NOT edit migration 055.
--
-- Confirmed production error (HTTP 400 / Postgres 42803):
--   column "ft.total_weight" must appear in the GROUP BY clause
--   or be used in an aggregate function
--
-- Root cause (CTE focus_selected_share):
--   Aggregates sum(fb.loading_weight) / count(*) while referencing
--   non-aggregated ft.total_weight from a CROSS JOIN to focus_totals.
--
-- Also fixes insights CTE insight_top referencing i.percentage when
-- top_items only exposes rank, consignee, weight, lr_count.
--
-- Business behavior, security, filters, and RPC signature unchanged.
--
...
begin
  ...
  -- Share of the selected material within the focus consignee's total
  -- loading weight. Aggregate selected-material rows first, then divide
  -- by focus_totals (single-row CTE) — avoids 42803 GROUP BY error.
    ...
    select
      case
        when f.focus_name is null then null
          ...
          select jsonb_build_object(
            'material', v_material,
            'weight', round(s.sel_weight, 3),
            'percentage', case
              when ft.total_weight > 0
                then round((s.sel_weight / ft.total_weight) * 100, 1)
              else 0
            end,
          ...
          cross join lateral (
            select
              coalesce(sum(fb.loading_weight), 0)::numeric as sel_weight,
              count(*)::integer as trip_count
            from focus_base fb
...
comment on function public.get_material_intelligence(text, text, text) is
  'Material Intelligence V1 (056): window-scoped loading_weight analytics by material. Fixes focus_selected_share GROUP BY and insight_top percentage. Requires lr view. Read-only.';
