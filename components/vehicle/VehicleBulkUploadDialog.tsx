...
import {
  parseAndValidateVehicleUpload,
  type VehicleUploadRow,
  type VehicleUploadRowError,
...
interface VehicleBulkUploadDialogProps {
  ...
  /** Reuses the list already loaded by VehicleListPage — no extra fetch. */
  existingVehicles: VehicleRecord[];
  onImported: () => void | Promise<void>;
}

export default function VehicleBulkUploadDialog({
  open,
  onOpenChange,
  existingVehicles,
  onImported,
}: VehicleBulkUploadDialogProps) {
  const [fileName, setFileName] = useState("");
  const [parsing, setParsing] = useState(false);
  const [hasParsed, setHasParsed] = useState(false);
  const [rows, setRows] = useState<VehicleUploadRow[]>([]);
  const [errors, setErrors] = useState<VehicleUploadRowError[]>([]);
  ...
  async function handleFileSelect(file: File) {
    ...
    try {
      setParsing(true);
      const result = await parseAndValidateVehicleUpload(file, existingVehicles);
      setRows(result.rows);
  ...
  async function handleImport() {
    ...
    try {
      ...
      for (const row of rows) {
        const created = await createVehicle(row.values);
        createdIds.push(created.id);
      }

      toast.success(`${rows.length} vehicle${rows.length === 1 ? "" : "s"} imported successfully.`);
      resetState();
      onOpenChange(false);
      await onImported();
    } catch (error) {
  ...
  return (
    <FormDialog
      ...
      description="Upload a completed template to import multiple vehicles at once."
      loading={importing}
      loadingText="Importing vehicles..."
      footer={
        <>
          <Button
            variant="outline"
          ...
          >
            {importing
              ? "Importing..."
              : `Import ${rows.length > 0 ? rows.length : ""} Vehicle${rows.length === 1 ? "" : "s"}`}
          </Button>
        </>
      }
    >
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <Button
            ...
            onClick={() => document.getElementById("vehicle-bulk-upload-input")?.click()}
          >
        ...
        {!parsing && hasParsed && errors.length === 0 && rows.length > 0 && (
          <p className="text-sm font-medium text-success">
            {rows.length} vehicle{rows.length === 1 ? "" : "s"} passed validation and{" "}
